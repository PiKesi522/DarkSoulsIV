...
public class CharacterState : MonoBehaviour
{
    ...
    void AnimationStateConfirm(){
        if(this.motionState == motionState.idle){
            ChangeAnimationState(PLAYER_IDLE);
        }
        else if (this.motionState == motionState.walkForward){
            ChangeAnimationState(PLAYER_WALK);
        }
        else if (this.motionState == motionState.runForward){
            ChangeAnimationState(PLAYER_RUN);
        }
        else if (this.motionState == motionState.useLeftHand_Attack1){
            ChangeAnimationState(PLAYER_ATTACK1);
        }
        else if (this.motionState == motionState.useLeftHand_Attack2){
            ChangeAnimationState(PLAYER_ATTACK2);
        }
        else if (this.motionState == motionState.useRightHand_Defend){
            ChangeAnimationState(PLAYER_DEFEND);
        }
        else if (this.motionState == motionState.die){
            ChangeAnimationState(PLAYER_DIE);
        }
    }
