...
public class CharacterStateNew : MonoBehaviour
{
    ...
    private float motionIntervalMAX = 8.0f;
    ...
    private bool recoverHit = false;
    void checkState(){
        if(isHit){
            if(!recoverHit){
                playHit += Time.deltaTime * 1.5f;
                if(playHit > 1f){
                    recoverHit = true;
                    playHit = 1f;
                }
            ...
            animator.SetFloat("isHit", playHit);
            animator.SetFloat("Rest", 1 - playHit);
            animator.SetFloat("isAttack", 0f);
            animator.SetFloat("isDefend", 0f);
            animator.SetFloat("isWalk", 0f);
            animator.SetFloat("isRun", 0f);
            ...
            Rest = (isAttack > 0.9f || isWalk > 0.9f || isDefend > 0.9f) ? 0f: 1f;
