...
public class CharacterState : MonoBehaviour
{
    ...
    void recoveryEndurance(){
        if(endurance.GetComponent<Slider>().value + 5f > 100f){
    ...
    void decreaseEndurance(float cost){
        if(endurance.GetComponent<Slider>().value - (cost + 5f) < 0){
            endurance.GetComponent<Slider>().value = 0;
        }
        else{
            endurance.GetComponent<Slider>().value -= (cost + 5f);
        }
    }
}
