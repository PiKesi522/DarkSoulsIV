    ...
    void Update()
    {
        ...
        if(Input.GetMouseButtonDown(0)){
            ...
            if(motionInterval < 0.1f){
                ...
                attackSeq = !attackSeq;
                animator.SetInteger("MotionState", attackSeq ? motionState.useLeftHand_Attack1 : motionState.useLeftHand_Attack2);
            }else{
                Debug.Log("Too fast");
            }
        }
            ...
            animator.SetInteger("MotionState", motionState.useRightHand_Defend);
        }
        else if(Input.GetKey(KeyCode.W)){
            if(Input.GetKey(KeyCode.LeftShift)){
                Debug.Log("run");
                // this.presentState = motionState.runForward;
                animator.SetInteger("MotionState", motionState.runForward);
            }
            else{
                Debug.Log("walk");
                // this.presentState = motionState.walkForward;
                animator.SetInteger("MotionState", motionState.walkForward);
            }
        }else{
            animator.SetInteger("MotionState", motionState.idle);
            // this.presentState = motionState.idle;
        }
