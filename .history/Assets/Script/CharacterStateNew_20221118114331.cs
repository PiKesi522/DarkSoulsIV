...
public class CharacterStateNew : MonoBehaviour
{
    ...
    void checkState(){
        ...
        // Rest = 1 - isBattle - isAttack - isDefend - isWalk - isRun;
        Rest = isBattle > 0.9f || isWalk > 0.9f ? 0f: 1f;
        ...
        isBattle = isAttack + isDefend > 0.9f ? 1f : 0f;

        // if(isBattle < 0.1f && isWalk < 0.1f){
        //     animator.SetInteger("State", (int)(motionState.idle));
        // }

        animator.SetFloat("isBattle", isBattle);
        animator.SetFloat("isAttack", isAttack < 0.16f ? 0f : 1f);
        animator.SetFloat("isDefend", isDefend < 0.8f ? 0f : 1f);
