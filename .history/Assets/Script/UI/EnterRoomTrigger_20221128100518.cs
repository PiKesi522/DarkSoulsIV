...
public class EnterRoomTrigger : MonoBehaviour
{
    ...
    private void OnTriggerExit(Collider other) {
        if(other.gameObject.name == "PlayerCore"){
            ...
            this.transform.parent.Find("DoorLock").gameObject.GetComponent<BoxCollider>().enabled = true;
            // this.gameObject.GetComponents<BoxCollider>()[1].enabled = false;
            AM.GetComponent<AudioManager>().AudioPlayBoss();
            AM.transform.Find("LoopBGM").gameObject.GetComponent<AudioSource>().Stop();
            UI.transform.Find("BossName").gameObject.SetActive(true);
            UI.transform.Find("BossName").GetComponent<TextFade>().TextFadeOut();
            UI.transform.Find("BossHealthBar").gameObject.SetActive(true);
            UI.transform.Find("BossHealthBar").GetComponent<BarFade>().BarFadeOut();
        }
    }
}
