...
public class BossEvent : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        // canvas = this.gameObject.transform.Find("BossCanvas").gameObject;

        // weapon = transform.Find("root/pelvis/Weapon").gameObject;
        weapon.GetComponent<BoxCollider>().enabled = false;
        
        // canvas.transform.Find("BossHealthBar").value = HP;
            ...
            if(isAttack){
                if(motionGap < 0.2f){
                    if(Random.Range(0,10) == 0){
                        Debug.Log("Do attack");
                        motionGap = Random.Range(30f, motionGapMax);
                        attackSeq = !attackSeq;
