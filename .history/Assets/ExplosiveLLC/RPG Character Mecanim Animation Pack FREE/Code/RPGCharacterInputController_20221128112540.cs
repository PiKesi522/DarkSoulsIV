...
namespace RPGCharacterAnims
{
	...
	public class RPGCharacterInputController : MonoBehaviour
    {
		...
		// public CharacterStateBeta state;

        private void Awake()
        {
            rpgCharacterController = GetComponent<RPGCharacterController>();
			Lock = GameObject.Find("TargetLockArea").gameObject;
			// state = this.gameObject.GetComponent<CharacterStateBeta>();
        ...
        private void Update()
        {
			...
			if(this.gameObject.GetComponent<RPGCharacterController>().target == null){
				inputAiming = false;
			}
		...
		public bool SetAimInput(bool aim) => inputAiming = aim;
		...
		public void Rolling()
        {
            if (!inputRoll ||
                ...
                !rpgCharacterController.CanStartAction(HandlerTypes.DiveRoll));
				{ return; }

			if(this.gameObject.GetComponent<CharacterStateBeta>().canRoll()){
