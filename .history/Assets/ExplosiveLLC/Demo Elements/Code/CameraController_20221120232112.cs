...
namespace RPGCharacterAnims
{
	...
	public class CameraController:MonoBehaviour
	{
		...
		private void Update()
		{
			...
			// Rotate cam.
			if (inputRotateL) { rotate = -0.1f; }
			else if (inputRotateR) { rotate = 0.1f; }
			else { rotate = 0; }
			...
			if(player.GetComponent<RPGCharacterInputController>().HasAimInput()) {
                ...
                angles.y = player.transform.rotation.eulerAngles.y - 360;
                // transform.rotation = Quaternion.Euler(angles);

				// var pos = transform.position;
				// Debug.Log(pos);
				// pos.x = player.transform.position.x - 4f * Mathf.Sin(player.transform.rotation.eulerAngles.y);
				// pos.z = player.transform.position.z + 4f * Mathf.Cos(player.transform.rotation.eulerAngles.y);
				// Debug.Log(pos);
				...
				float dis = newDistance * Mathf.Cos(angles.y);
				float height = newDistance * Mathf.Sin(angles.y);
				...
				transform.position = Vector3.Lerp(transform.position, cameraPos, Time.fixedDeltaTime * 1f);
