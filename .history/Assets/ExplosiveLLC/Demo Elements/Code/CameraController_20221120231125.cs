	...
	public class CameraController:MonoBehaviour
	{
		...
		private void Update()
		{
			...
			if(player.GetComponent<RPGCharacterInputController>().HasAimInput()) {
				...
				pos.x = player.position.x - 4f * Mathf.Sin(player.transform.rotation.eulerAngles.y);
				pos.z = player.position.z + 4f * Mathf.Cos(player.transform.rotation.eulerAngles.y);
