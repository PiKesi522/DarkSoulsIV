...
namespace RPGCharacterAnims
{
	...
	public class CameraController:MonoBehaviour
	{
		...
		private void Update()
		{
			...
			// Rotate cam.
			if (inputRotateL) { rotate = -0.2f; }
			else if (inputRotateR) { rotate = 0.2f; }
			else { rotate = 0; }
			...
			if(Input.GetMouseButtonDown(2)){
                player.GetComponent<RPGCharacterInputController>().SetAimInput(false);
				
                transform.rotation =  player.transform.rotation.eulerAngles;

				var posY = (player.transform.rotation.eulerAngles.y + 360) % 360;
				var newPos = transform.position;
				Debug.Log(newPos);
				newPos.x -= Mathf.Cos(posY) * 4f;
				newPos.z -= Mathf.Sin(posY) * 4f;
				Debug.Log(newPos);
